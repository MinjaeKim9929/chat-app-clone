	...
	return (
		selectedUser && (
			...
			>
				...
				<hr className="border-[#ffffff50] my-8" />

				<div className="px-5 text-xs">
					<p>Media</p>
					<div className="mt-4 max-h-[280px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80">
						{msgImages.map((url, index) => (
							<div key={index} onClick={() => window.open(url)} className="cursor-pointer rounded">
								<img src={url} alt="" className="w-full rounded-md aspect-square object-cover" />
							</div>
						))}
					</div>
				</div>
